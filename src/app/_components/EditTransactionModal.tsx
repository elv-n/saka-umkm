...
export default function EditTransactionModal({
  ...
  return (
    <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 transition-opacity">
      ...
      {/* Modal Card */}
      <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl  border border-zinc-200  animate-in fade-in zoom-in-95 duration-200">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-bold text-base text-zinc-900">Ubah Transaksi</h3>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600   text-xs font-bold transition flex items-center justify-center"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        {error && (
          <p className="mb-3 rounded-lg bg-red-50 p-2.5 text-center text-sm font-semibold text-red-600">
            {error}
          </p>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          {/* Nama Barang/Jasa */}
          <div>
            <label htmlFor="edit-item" className="block text-xs font-bold uppercase tracking-wider text-zinc-500  mb-1">
              Nama Barang / Jasa
            </label>
            <input
              id="edit-item"
              ...
              className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2.5 text-base md:text-xs outline-none focus:border-emerald-500"
          ...
          {/* Kategori */}
          <div>
            <label htmlFor="edit-kategori" className="block text-xs font-bold uppercase tracking-wider text-zinc-500  mb-1">
              Kategori
            </label>
            <input
              id="edit-kategori"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label htmlFor="edit-qty" className="block text-xs font-bold uppercase tracking-wider text-zinc-500  mb-1">
                Jumlah (Qty)
              </label>
              <input
                id="edit-qty"
            ...
            <div>
              <label htmlFor="edit-harga" className="block text-xs font-bold uppercase tracking-wider text-zinc-500  mb-1">
                Harga Satuan (Rp)
              </label>
              <input
                id="edit-harga"
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label htmlFor="edit-total" className="block text-xs font-bold uppercase tracking-wider text-zinc-500">
                Total Nominal (Rp)
              </label>
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2.5 text-base md:text-xs outline-none focus:border-emerald-500    disabled:opacity-60"
          ...
          <div className="flex gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-lg border border-zinc-300 py-2.5 text-sm font-semibold hover:bg-zinc-50   transition"
              disabled={loading}
            >
              Batal
            </button>
