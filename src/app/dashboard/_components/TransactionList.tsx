    ...
    <>
      <ul className="divide-y divide-zinc-200 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
        {transaksi.map((t) => (
          <li
            key={t.id}
            className="flex items-center justify-between px-3.5 py-2.5 hover:bg-zinc-50/55 transition"
          >
            <div className="min-w-0 flex-1 pr-2">
              <p className="text-xs sm:text-sm font-semibold truncate text-zinc-800">
                {t.item}
              </p>
            ...
            <div className="flex flex-col items-end gap-0.5 shrink-0">
              <span
                className={
                  t.tipe === "PEMASUKAN"
                    ? "text-xs sm:text-sm font-extrabold text-emerald-600"
                    : "text-xs sm:text-sm font-extrabold text-rose-600"
                }
              >
                {t.tipe === "PEMASUKAN" ? "+" : "-"}
